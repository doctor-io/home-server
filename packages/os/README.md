...
- **Purpose**: Stabilize mDNS `.local` hostname resolution
- **Key settings**:
  - `deny-interfaces=docker0,lo`: Exclude virtual interfaces (Docker bridges, loopback)
  - `use-iff-running=no`: Ignore IFF_RUNNING flag — prevents spurious restarts on WiFi
  - `use-ipv6=no`: IPv4 only, avoids binding to Docker IPv6 link-local addresses
