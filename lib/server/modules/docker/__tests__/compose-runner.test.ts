...
`;
    ...
    const appDataRoot = "/Users/ahmedtabib/Code/home-server/DATA/AppData";
    ...
    expect(normalized.composeContent).toContain(
      '- "/Users/ahmedtabib/Code/home-server/DATA/AppData/big-bear-2fauth_data:/2fauth"',
    );
    ...
    expect(Array.from(normalized.bindMountDirectories)).toContain(
      "/Users/ahmedtabib/Code/home-server/DATA/AppData/big-bear-2fauth_data",
    );
  });
...
`;
    ...
    const stacksRoot = "/DATA/AppData";
    const appDataRoot = "/DATA/AppData";
    const normalized = normalizeComposeStorageBindings(
      compose,
      stacksRoot,
      appDataRoot,
    ...
    expect(normalized.composeContent).toContain(
      '- "/DATA/AppData/home-assistant/var/lib/homeassistant/media:/var/lib/homeassistant/media"',
    );
    expect(normalized.composeContent).not.toContain(
      "big-bear-home-assistant_config:",
    );
    ...
    expect(Array.from(normalized.bindMountDirectories)).toContain(
      "/DATA/AppData/home-assistant/config",
    );
    expect(Array.from(normalized.bindMountDirectories)).toContain(
      "/DATA/AppData/home-assistant/var/lib/homeassistant/media",
    );
  });
...
`;
    ...
    expect(normalized.composeContent).toContain(
      '- "/DATA/AppData/cloudflared/data:/data"',
    );
    expect(Array.from(normalized.bindMountDirectories)).toContain(
      "/DATA/AppData/cloudflared/config",
    );
...
`;
  ...
  it("collects app-data ownership overrides for n8n bind mounts", () => {
    const compose = `
services:
  n8n:
...
`;
    ...
    expect(collectComposeBindMountOwnershipOverrides(compose)).toEqual([
      {
        directoryPath: "/DATA/AppData/n8n",
        uid: 1000,
        gid: 1000,
      },
    ]);
...
`;
  ...
  it("resolves n8n ownership overrides with AppID interpolation", () => {
    const compose = `
services:
  n8n:
    image: n8nio/n8n:1.123.0
...
`;
  ...
  it("uses explicit numeric service user ownership before image-specific rules", () => {
    const compose = `
services:
  app:
    image: grafana/grafana:12.1.4
    ...
    volumes:
      - type: bind
        source: /DATA/AppData/app/data
        target: /var/lib/grafana
`;

    expect(collectComposeBindMountOwnershipOverrides(compose)).toEqual([
      {
        directoryPath: "/DATA/AppData/app/data",
        uid: 1234,
        gid: 5678,
      },
    ]);
  ...
  it("uses PUID and PGID environment values when service user is not set", () => {
    const compose = `
services:
  app:
    ...
    volumes:
      - type: bind
        source: /DATA/AppData/app/config
        target: /config
`;

    expect(collectComposeBindMountOwnershipOverrides(compose)).toEqual([
      {
        directoryPath: "/DATA/AppData/app/config",
        uid: 1001,
        gid: 1002,
      },
    ]);
  ...
  it("uses list-style PUID and PGID environment values", () => {
    const compose = `
services:
  app:
    image: lscr.io/linuxserver/app:latest
...
`;
  ...
  it("removes the installed stack directory during uninstall cleanup without deleting app data by default", async () => {
    ...
    // Use os.tmpdir() so bindDir is outside stackDir — avoids false ENOENT when
    // the recursive stackDir removal also wipes a nested bindDir (happens in dev
    // where stacksRoot and appDataRoot both resolve to DATA/AppData).
    const bindDir = await mkdtemp(path.join(os.tmpdir(), "homeio-test-bind-"));
