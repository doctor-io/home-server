...
async function runUpdateOperation(
  ...
  const composeInput = {
    ...
    envPath: resolveEnvPathFromComposePath(existingStack.composePath),
  ...
  try {
    installedComposeSource = await readFile(existingStack.composePath, "utf8");
  } catch {
  ...
  const backupPath = `${existingStack.composePath}.bak`;
  ...
  try {
    ...
    await copyFile(existingStack.composePath, backupPath);
    backupPrepared = true;
    await writeFile(existingStack.composePath, nextComposeSource, "utf8");
    ...
    await patchOperationAndEmit({
      ...
      progressPercent: 32,
      step: "pull-images",
      message: "Resolving updated images",
    });

    const images = await extractComposeImages(composeInput);
    const imageProgress = new Map<string, number>();
    for (const image of images) {
      imageProgress.set(image, 0);
    }
      ...
      await pullDockerImage(image, async (event) => {
        ...
        const pullPercent =
          totals.length === 0
            ? 100
            : totals.reduce((accumulator, value) => accumulator + value, 0) / totals.length;
        const operationPercent = 32 + pullPercent * 0.5;
    ...
    if (backupPrepared) {
      try {
        await copyFile(backupPath, existingStack.composePath);
        await runComposeUp(composeInput);
      } catch (recoveryError) {
        logServerAction({
          level: "error",
