...
export async function moveToTrash(pathInput: string) {
  ...
  try {
    ...
    // Use the public virtual path (e.g. "Trash/name") rather than the
    // filesystem-relative path (e.g. ".Trash/name") so that DB lookups in
    // restoreFromTrash and deleteFromTrash — which use trashPath.relativePath
    // from the path resolver — always match.
    const trashBasename = path.basename(trashTarget);
    const trashPath = `${trashRoot.relativePath}/${trashBasename}`;
...
export async function restoreFromTrash(
  ...
  try {
    ...
    const trashRoot = await assertTrashPath("Trash");
    await pruneEmptyAncestors(trashRoot.absolutePath, trashPath.absolutePath);
