...
function WifiStrengthIcon({ strength }: { strength: string }) {
  if (strength === "strong") return <SignalHigh className="size-3.5 text-status-green" />
  if (strength === "medium") return <SignalMedium className="size-3.5 text-status-amber" />
  return <SignalLow className="size-3.5 text-status-red" />
}
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  <span className="text-sm text-foreground truncate">{net.name}</span>
                  {net.secured && <Lock className="size-2.5 text-muted-foreground shrink-0" />}
                </div>
                {net.connected && (
                  <span className="text-[10px] text-primary">Connected</span>
                )}
              </div>
              {net.connected && <Check className="size-4 text-primary shrink-0" />}
            </button>
          ))}
    ...
    >
        ...
        <div className="space-y-3">
          <div>
            ...
            <div className="h-2 rounded-full bg-muted overflow-hidden">
              <div className="h-full rounded-full bg-status-green" style={{ width: "92%" }} />
            </div>
          </div>

          <div className="grid grid-cols-2 gap-3 pt-1">
            <div className="flex flex-col gap-0.5">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Runtime</span>
              <span className="text-sm font-medium text-foreground">2h 14m</span>
            </div>
            <div className="flex flex-col gap-0.5">
            ...
            <div className="flex flex-col gap-0.5">
              <span className="text-[10px] text-muted-foreground uppercase tracking-wider">Voltage</span>
            ...
            <div className="flex flex-col gap-0.5">
              ...
              <span className="text-sm font-medium text-foreground">APC 1500</span>
            </div>
          </div>
        </div>
      </div>
      <div className="border-t border-glass-border px-4 py-2.5">
        <span className="text-[10px] text-muted-foreground">
          No battery detected refers to server itself. UPS provides backup power.
        </span>
      </div>
    </div>
  )
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-glass-border px-4 py-3">
        <p className="text-[10px] text-muted-foreground uppercase tracking-wider mb-2">5-Day Forecast</p>
        <div className="flex items-center justify-between gap-1">
          {forecast.map((d) => (
            <div key={d.day} className="flex flex-col items-center gap-1 flex-1">
              <span className="text-[10px] text-muted-foreground">{d.day}</span>
              <d.icon className="size-4 text-foreground/70" />
              <span className="text-xs font-medium text-foreground">{d.temp}°</span>
            </div>
          ))}
        </div>
      </div>
...
const notifications: Notification[] = [
  { id: "1", title: "Immich Update", message: "Version 1.94 is ready to install", time: "5 min ago", read: false },
  { id: "2", title: "Disk Warning", message: "Storage pool at 85% capacity", time: "32 min ago", read: false },
  { id: "3", title: "Backup Complete", message: "Weekly backup finished successfully", time: "2h ago", read: true },
  { id: "4", title: "Pi-hole", message: "Blocked 14,283 queries today", time: "3h ago", read: true },
]
  ...
  return (
    ...
    >
      ...
      <div className="border-t border-glass-border px-3 py-2">
        <button className="text-xs text-primary hover:underline cursor-pointer">View all notifications</button>
      </div>
    </div>
  )
}
...
export function StatusBar() {
  ...
  return (
    <header className="fixed top-3 left-1/2 -translate-x-1/2 z-50 w-auto max-w-[96vw]">
      <div className="flex items-center gap-2 px-4 py-2 bg-dock backdrop-blur-2xl border border-glass-border rounded-2xl shadow-2xl shadow-black/30">
        ...
        <div className="relative hidden sm:block">
          <button
            ...
            className="flex items-center gap-2 px-3 py-1 rounded-lg bg-glass-highlight hover:bg-secondary/40 transition-colors cursor-pointer"
        ...
        <div className="flex items-center gap-1.5">
          ...
          <div className="relative">
            <button
              onClick={() => { closeAll(); setWeatherOpen(!weatherOpen); }}
            ...
            >
              ...
              <span className="text-[11px] text-foreground font-medium">22°</span>
            </button>
            {weatherOpen && <WeatherPopover onClose={() => setWeatherOpen(false)} />}
          </div>
          ...
          <div className="relative">
            <button
              onClick={() => { closeAll(); setBatteryOpen(!batteryOpen); }}
            ...
            >
              ...
              <span className="text-[10px] text-muted-foreground font-medium">92%</span>
            </button>
            {batteryOpen && <BatteryPopover onClose={() => setBatteryOpen(false)} />}
          </div>
          ...
          <div className="relative">
            ...
            >
              ...
              {unreadCount > 0 && (
                <span className="absolute -top-0.5 -right-0.5 size-3.5 rounded-full bg-primary text-[8px] text-primary-foreground flex items-center justify-center font-bold">
                  {unreadCount}
                </span>
              )}
            </button>
