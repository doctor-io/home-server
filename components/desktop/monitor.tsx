...
export function Monitor() {
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      ...
      setCpuHistory((prev) => [
        ...prev.slice(-23),
        nudge(prev[prev.length - 1] || 20, 8, 72),
      ]);
      setMemHistory((prev) => [
        ...prev.slice(-23),
        nudge(prev[prev.length - 1] || 34, 22, 79, 1.7),
      ]);
            ...
            {filteredProcesses.map((p) => (
              ...
              >
                <div className="flex min-w-0 items-center gap-2">
                  <span className="flex size-6 shrink-0 items-center justify-center rounded-md bg-secondary/60 text-xs font-semibold text-foreground">
                    {p.name[0]}
                  </span>
      ...
      {tab === "network" && (
        <div className="flex-1 overflow-y-auto p-4">
          <div className="grid gap-3">
            ...
            <div className="rounded-xl border border-glass-border bg-glass p-3">
              <div className="mb-2 flex items-center gap-2 text-xs font-medium text-foreground">
                <Activity className="size-3.5 text-primary" /> Active
                Connections
              </div>
              <div className="grid grid-cols-4 gap-2">
                ...
                <div className="rounded-lg bg-secondary/40 p-2 text-center">
                  ...
                  <p className="text-xs text-muted-foreground">TIME_WAIT</p>
                </div>
                <div className="rounded-lg bg-secondary/40 p-2 text-center">
