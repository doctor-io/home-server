...
vi.mock("@/modules/files/components/network-storage-dialog", () => ({
...
describe("FileManager sharing and navigation", () => {
  ...
  it("shows Unshare Folder action when folder is already shared", () => {
    mockUseFilesDirectory.mockReturnValue(
      mockDirectory([
        {
          name: "FolderToShare",
  ...
  it("copies shared folder path as smb url when network address is available", async () => {
    ...
    fireEvent.click(screen.getByRole("button", { name: "Shared" }));
    fireEvent.contextMenu(screen.getByRole("button", { name: /portainer/i }), {
      clientX: 120,
      clientY: 120,
    });
    ...
    await waitFor(() => {
      expect(writeText).toHaveBeenCalledWith(
        "smb://192.168.1.12/DATA/Shared/portainer",
      );
    });
  });
  ...
  it("shows Empty Trash in header and deletes all trash entries", async () => {
    ...
    fireEvent.click(
      await screen.findAllByRole("button", { name: /empty trash/i }).then(
        (buttons) => buttons.at(-1)!,
      ),
    );
