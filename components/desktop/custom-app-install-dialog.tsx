...
type CustomAppInstallDialogProps = {
  open: boolean;
...
export function CustomAppInstallDialog({
  open,
  isSubmitting,
  error,
  onOpenChange,
  onSubmit,
}: CustomAppInstallDialogProps) {
  ...
  useEffect(() => {
    ...
    if (!source.trim()) {
      setSource(defaultSourceByType[sourceType]);
    }
  ...
  return (
    <Dialog open={open} onOpenChange={handleOpenChange}>
      <DialogContent className="max-w-2xl bg-popover/95 border-glass-border">
        <DialogHeader>
          <DialogTitle>Install Custom App</DialogTitle>
          <DialogDescription>
