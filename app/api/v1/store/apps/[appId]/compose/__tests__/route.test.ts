...
describe("GET /api/v1/store/apps/:appId/compose", () => {
  it("returns raw catalog compose with primary service metadata", async () => {
    ...
    const response = await GET(
      new Request(
        "http://localhost/api/v1/store/apps/immich/compose?source=catalog",
      ),
      {
    ...
    expect(json.data.primary.image).toBe(
  ...
  it("returns 400 for invalid source", async () => {
    const response = await GET(
      new Request(
        "http://localhost/api/v1/store/apps/immich/compose?source=bad",
      ),
      {
        params: Promise.resolve({ appId: "immich" }),
      },
  ...
  it("returns installed compose from disk", async () => {
    ...
    vi.mocked(resolveStoreStacksRoot).mockReturnValue("/DATA/AppData");
    vi.mocked(readFile)
    ...
    const response = await GET(
      new Request(
        "http://localhost/api/v1/store/apps/home-assistant/compose?source=installed",
      ),
      {
    ...
    expect(response.status).toBe(200);
    expect(json.data.compose).toContain(
  ...
  it("returns installed_compose_missing when installed stack is not found", async () => {
    ...
    const response = await GET(
      new Request(
        "http://localhost/api/v1/store/apps/missing/compose?source=installed",
      ),
      {
  ...
  it("returns custom catalog compose from stored content", async () => {
    ...
    const response = await GET(
      new Request(
        "http://localhost/api/v1/store/apps/custom-nginx/compose?source=catalog",
      ),
      {
