...
export function useStoreOperation(operationId: string | null) {
  ...
  const [latestEvent, setLatestEvent] = useState<StoreOperationEvent | null>(
    null,
  );
      ...
      onEvent: (event) => {
        ...
        if (isTerminalStatus(event.status)) {
          void Promise.all([
            ...
            queryClient.invalidateQueries({
              queryKey: queryKeys.storeApp(event.appId),
            }),
            queryClient.invalidateQueries({
              queryKey: queryKeys.storeOperation(operationId),
            }),
          ]);
        }
      },
      onError: (_error) => {
        // SSE stream errors are transient (reconnects, auth expiry).
        // The operation state from the last successful event is preserved in the query cache.
      },
    });
