...
export function NetworkStorageDialog({
  ...
  return (
    ...
    >
      <div
        className={`relative flex h-[50vh] w-full max-w-6xl flex-col ${FILES_MENU_SHELL}`}
      ...
      >
            ...
            <div className="min-h-0 flex-1 space-y-2 overflow-y-auto">
              {sharesQuery.isLoading ? (
                ...
                sharesQuery.data.map((item) => (
                  <div
                    key={item.id}
                    className="rounded-md border border-glass-border bg-background/55 p-2"
                  >
        ...
        {isAddFormOpen ? (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black/45 p-4">
            <section className={`w-full max-w-lg p-4 ${FILES_PANEL_SHELL}`}>
              <div className="mb-3 flex items-center gap-2">
